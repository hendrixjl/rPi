...
bool gpio_event_detected(gpio_pin_t pin)
{
	...
	printf("here. GPIO_BAR[GPEDS0]=%08X.  state=%08X", (unsigned)&GPIO_BAR[word], GPIO_BAR[word]);
...
void gpio_clear_event_detected(gpio_pin_t pin)
{
	...
	printf("here. GPIO_BAR[GPEDS0]=%08X.  pin=%d", (unsigned)&GPIO_BAR[word], pinInWord);
...
void gpio_set_event_detect(gpio_pin_t pin, event_type_t event_type)
{
	...
	switch (event_type)
	{
		...
		case FALLING_EDGE_DETECT:
			GPIO_BAR[word+(GPFEN0-GPREN0)] |= (1<<pinInWord);
			printf("here. GPIO_BAR[GPFEN0]=%08X.  pin=%d", (unsigned)&GPIO_BAR[word+(GPFEN0-GPREN0)], pinInWord);
			break;
